...
create or replace package body PC8 as
    ...
    begin
        ...
        v_codigo := TablaNuevaTransaccion(p_central).aeros.first;
        while v_codigo is not null loop
            if TablaEstadoAnterior.exists(v_codigo) then
                old_central := TablaEstadoAnterior(v_codigo).central;
                old_modelo := TablaEstadoAnterior(v_codigo).modelo;
            ...
            v_codigo := TablaNuevaTransaccion(p_central).aeros.next(v_codigo);
        end loop;
    ...
    begin
        v_cant_centrales := TablaNuevaTransaccion.count;
        if v_cant_centrales > 0 then
            v_central := TablaNuevaTransaccion.first;
            while v_central is not null loop
                ...
                v_central := TablaNuevaTransaccion.next(v_central);
            end loop;
        end if;
